...
public class NestableCondition<ACTUAL, NESTED> extends Join<ACTUAL> {
     ...
     * @param <ACTUAL> the type of object the resulting condition accepts
     */
    public static <ACTUAL> Condition<ACTUAL> nestable(String descriptionPrefix, Collection<Condition<? super ACTUAL>> conditions) {
        return new NestableCondition<>(descriptionPrefix, conditions.stream());
    }
    
    private NestableCondition(String descriptionPrefix, Stream<Condition<? super NESTED>> conditions, Function<ACTUAL, NESTED> extractor) {
    ...
    private static <ACTUAL, NESTED> List<Condition<ACTUAL>> compose(Stream<Condition<? super NESTED>> conditions,
                                                                    Function<ACTUAL, NESTED> extractor) {
        return conditions.map(condition -> compose(condition, extractor)).collect(toList());
    }

    private static <ACTUAL, NESTED> Condition<ACTUAL> compose(Condition<? super NESTED> condition, Function<ACTUAL, NESTED> extractor) {
